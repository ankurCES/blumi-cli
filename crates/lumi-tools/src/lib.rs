//! Built-in tools for the lumi agent.
//!
//! Each tool implements `lumi_core::TypedTool` and is wrapped in `Typed` to
//! become a `Tool`. [`register_builtin_tools`] installs the default set into a
//! registry; the generic execution pipeline lives in `lumi-core`.
...
use lumi_core::{ToolRegistry, Typed};
use std::sync::Arc;
...
pub(crate) mod testutil {
    use lumi_core::{EventEmitter, Interactor, ToolContext};
    use lumi_exec::LocalExecutor;
    use lumi_protocol::SessionId;
    use std::path::Path;
    use std::sync::Arc;
    use tokio::sync::mpsc;
