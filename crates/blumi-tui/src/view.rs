...
fn build_lines(model: &Model, width: usize, theme: &Theme) -> Vec<Line<'static>> {
    ...
    for entry in &model.entries {
        match entry {
            ...
            Entry::Assistant(t) => {
                push_wrapped(&mut lines, t, width, theme.body(), "");
            }
            Entry::Tool {
                name,
                summary,
    ...
    if let Some(s) = &model.streaming {
        push_wrapped(&mut lines, s, width, theme.body(), "");
    }
    lines
}
...
fn render_status(model: &Model, f: &mut Frame, area: Rect, theme: &Theme) {
    let hint = if model.pending.is_some() {
        "[a] allow once   [s] allow session   [d] deny"
    } else if model.busy {
        "esc cancel · ctrl+c quit"
    } else {
        "enter send · ctrl+j newline · tab focus · pgup/pgdn scroll · ctrl+c quit"
    };
    f.render_widget(
...
fn render_approval(model: &Model, f: &mut Frame, area: Rect, theme: &Theme) {
    ...
    if let Some(diff) = &p.diff {
        ...
        for l in diff.lines().take(12) {
            ...
            lines.push(Line::from(Span::styled(truncate(l, width), style)));
        }
    }
    lines.push(Line::raw(""));
    lines.push(Line::from(Span::styled(
