...
/// The `/heal` overlay: self-healing recovery + evolution summary.
fn render_heal(model: &Model, f: &mut Frame, area: Rect, theme: &Theme) {
    let Some(text) = &model.heal_view else {
        return;
    };
