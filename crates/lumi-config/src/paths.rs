...
/// All the paths lumi reads/writes at runtime.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct Paths {
    /// The lumi data home, default `~/.lumi`.
    pub home: PathBuf,
    /// SQLite database, `<home>/lumi.db`.
    pub db: PathBuf,
...
impl Paths {
    /// Resolve all paths. `home_override` (e.g. from `LUMI_HOME`) wins over the
    /// default `~/.lumi`; if no home can be found, falls back to `./.lumi`.
    pub fn resolve(home_override: Option<PathBuf>, working_dir: impl AsRef<Path>) -> Self {
        let home = home_override
            .or_else(|| dirs::home_dir().map(|h| h.join(".lumi")))
            .unwrap_or_else(|| working_dir.as_ref().join(".lumi"));

        Paths {
            db: home.join("lumi.db"),
...
mod tests {
    ...
    fn resolve_with_override() {
        let p = Paths::resolve(Some(PathBuf::from("/data/lumi")), "/work/proj");
        assert_eq!(p.home, PathBuf::from("/data/lumi"));
        assert_eq!(p.db, PathBuf::from("/data/lumi/lumi.db"));
        assert_eq!(p.memory_md(), PathBuf::from("/data/lumi/memory/MEMORY.md"));
